...
function startMove(friend) {
  // eslint-disable-line no-unused-vars
  fromAreas = findUnitsAreas(friend)
  hightlightUnitAreas(fromAreas)
  addMoveOriginListeners(fromAreas)
}

function findUnitsAreas(friend) {
  let allOrigins = []
  for (let area of game._allAreas) {
    if (area._control === friend) {
      if (
...
function moveOriginHandler(event) {
  ...
  mapMap.children[originId].addEventListener("click", rotateUnitsHandler, false)
}
